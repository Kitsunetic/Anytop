...
def avg_per_frame_dist(motion1, motion2, norm):
    ...
    if norm == "fro":
        ...
        return (
            torch.norm(motion2[:min_len].view(-1, n_joints, 3) - motion1[:min_len].view(-1, n_joints, 3), p=2, dim=-1)
            .mean()
            .cpu()
            .numpy()
        )
...
def pos_avg_l2(motion1, motion2):
    n_joints = motion1.shape[-1] // 3
    return (
        torch.norm(motion2.view(*motion2.shape[:2], n_joints, 3) - motion1.view(*motion1.shape[:2], n_joints, 3), p=2, dim=-1)
        .mean(dim=-1)
        .cpu()
        .numpy()
    )
