...
def npy2data(npy_file_path, mode):
    ...
    try:
        ...
        anim = anim["motion_raw"].transpose(
            0, 3, 1, 2
        )  # n_samples x n_joints x n_feats x n_frames  ==>  n_samples x n_frames x n_joints x n_feats
    y_root = anim[..., 0, 1]
    anim = anim[
        ..., 1:, :
    ]  # [1:] means excluding the root # [n_frames, n_joints, (3 ric pos, 6 rot, 3 linear vel, 1 foot contact)]
    if mode == "rot":
        anim_out = torch.from_numpy(anim[..., 3:9]).reshape(anim.shape[:-2] + (-1,))
    elif mode == "loc":
        anim_out = torch.from_numpy(anim[..., :3])
        anim_out[..., 1] = (
            anim_out[..., 1] - y_root[..., np.newaxis]
        )  # subtract y_root from y of each joint, as they are all absolute
...
def character2data(dir_path, character_name, mode):
    ...
    bvh_files = [
        os.path.join(dir_path, f) for f in os.listdir(dir_path) if f.endswith(file_ext) and f.split("_")[0] == character_name
    ]
    if len(bvh_files) == 0:
...
def evaluate_character(args, character_name):
    ...
    for i in range(n_gt_samples):
        ...
        for j in range(n_repetitions):
            gt_intra_div_dist.append(
                avg_per_frame_dist(
                    gt_samples[i][gt_offsets[j, 0] : gt_offsets[j, 0] + tmin],
                    gt_samples[i][gt_offsets[j, 1] : gt_offsets[j, 1] + tmin],
                    norm=eval_type,
                )
            )
    ...
    for i in range(n_gen_samples):
        ...
        intra_div_dist.append(
            avg_per_frame_dist(
                gen_samples[i][offsets[0] : offsets[0] + tmin], gen_samples[i][offsets[1] : offsets[1] + tmin], norm=eval_type
            )
        )
    ...
    for j in range(n_repetitions):
        ...
        coverages.append(
            coverage(
                concat_list_of_tensors(unique_gen_subset), gt_samples, threshold=coverage_threshold, tmin=tmin, use_pos=use_pos
            )
        )
...
if __name__ == "__main__":
    ...
    log_file = os.path.join(
        os.path.dirname(args.eval_gen_dir),
