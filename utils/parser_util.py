...
def add_model_options(parser):
    ...
    group.add_argument("--latent_dim", default=128, type=int, help="Transformer/GRU width.")
    group.add_argument(
        "--cond_mask_prob",
        default=0.1,
        type=float,
    ...
    group.add_argument("--temporal_window", default=31, type=int, help="temporal window size")
    group.add_argument("--skip_t5", action="store_true", help="If passed, joints names wont be added to features")
    group.add_argument(
        "--value_emb", action="store_true", help="If passed, graph multihead attention learns GRPE value embeddings"
    )
...
def add_data_options(parser):
    ...
    group.add_argument(
        "--data_dir", default="", type=str, help="If empty, will use defaults according to the specified dataset."
    )
    group.add_argument(
        "--objects_subset",
        default="all",
...
def add_training_options(parser):
    ...
    group.add_argument("--save_dir", type=str, help="Path to save checkpoints and results.")
    group.add_argument("--model_prefix", type=str, help="Unique string at the beggining of the model name.")
    ...
    group.add_argument(
        "--eval_split", default="test", choices=["val", "test"], type=str, help="Which split to evaluate on during training."
    )
    group.add_argument("--eval_during_training", action="store_true", help="If True, will run evaluation during training.")
    group.add_argument(
        "--eval_rep_times", default=3, type=int, help="Number of repetitions for evaluation loop during training."
    )
    group.add_argument(
        "--eval_num_samples", default=1_000, type=int, help="If -1, will use all samples in the specified split."
    )
    group.add_argument("--log_interval", default=50, type=int, help="Log losses each N steps")
    group.add_argument("--save_interval", default=10_000, type=int, help="Save checkpoints and run evaluation each N steps")
    ...
    group.add_argument("--gen_num_samples", default=3, type=int, help="Number of samples to sample while generating")
    group.add_argument(
        "--gen_num_repetitions", default=2, type=int, help="Number of repetitions, per sample (text prompt/action)"
    )
    group.add_argument("--use_ema", action="store_true", help="If True, will use EMA model averaging.")
    group.add_argument("--balanced", action="store_true", help="Use balancing sampler for fairness between topologies")
...
def add_dift_options(parser):
    ...
    group.add_argument("--sample_ref", default="assets/Monkey___B2Attack_574.npy", type=str, help="sample bvh ref.")
    group.add_argument(
        "--sample_tgt", default=["assets/Scorpion___SlowForward_837.npy"], type=str, nargs="+", help="sample bvh tgt."
    )
    group.add_argument("--tmp_save_dir", default="", type=str, help="temporal save dir.")
    group.add_argument("--suffix", default="", type=str, help="file suffix.")
    group.add_argument(
        "--dift_type",
        default="spatial",
    ...
    group.add_argument("--layer", default=0, type=int, help="Layer to extract DIFT features from.")
    group.add_argument("--timestep", default=90, type=int, help="Timestep to extract DIFT features from.")
...
def add_edit_options(parser):
    ...
    group.add_argument("--samples", default=["assets/Ostrich___Attack_581.npy"], type=str, nargs="+", help="samples npy")
    group.add_argument(
        "--object_type",
        default="Flamingo",
        type=str,
    ...
    group.add_argument(
        ...
        default="",
        type=str,
        help="A string to be added to the file name to identify a specific change. Should start with '_'.",
    )
...
def add_render_options(parser):
    ...
    group.add_argument(
        "--subset",
        default="bipeds",
        choices=["quadropeds", "flying", "bipeds", "millipeds_snakes"],
        type=str,
        help="Object subset.",
    )
...
def add_evaluation_options(parser):
    ...
    group.add_argument(
        "--benchmark_path",
        default="eval/benchmarks/benchmark_all.txt",
        type=str,
        help="Path to benchmark character names. If empty, will use all excluding the characters_to_exclude",
    )
    group.add_argument(
        "--eval_gt_dir", default="dataset/truebones/zoo/truebones_processed/motions", type=str, help="Path to gt dir."
    )
    group.add_argument("--eval_gen_dir", required=True, type=str, help="Path to gen dir.")
    group.add_argument(
        "--characters_to_exclude",
        default="MouseyNoFingers,Mousey_m,Trex,SabreToothTiger,Raptor2",
        type=str,
        help="Comma separated list of characters to exclude. The default is character with more than 40 motions.",
    )
    group.add_argument(
        "--unique_str",
        default="",
...
def process_new_skeleton_args():
    ...
    group.add_argument("--object_name", required=True, type=str, help="A character's indicative name")
    group.add_argument(
        "--bvh_dir",
        required=True,
        type=str,
    ...
    group.add_argument("--save_dir", required=True, type=str, help="Output directory.")
    group.add_argument(
        "--face_joints_names",
        default=["RLeg1", "LLeg1", "RArm1", "LArm1"],
        type=str,
        nargs=4,
