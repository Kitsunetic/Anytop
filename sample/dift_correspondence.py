...
def create_batch_from_motion_paths(motion_paths, cond_dict, temporal_window, t5_conditioner, max_joints):
    ...
    for motion_path in motion_paths:
        ...
        batches.append(
            create_sample_in_batch(motion, object_type, cond_dict_object_type, temporal_window, t5_conditioner, max_joints)
        )
...
def process_object_type(object_type, motion_path, cond):
    ...
    if os.path.isdir(motion_path):
        all_motions = [
            pjoin(motion_path, f) for f in os.listdir(motion_path) if f.endswith(".bvh") and f.startswith(f"{object_type}__")
        ]
...
def vis_dift(t, layer, activations, cond_ref, cond_tgt, motion_ref, motion_tgt, model_path, dift_type="spatial"):
    ...
    vec_norms = torch.norm(tgt_activations, dim=-1)[..., None] @ torch.norm(ref_activations, dim=-1)[..., None].transpose(
        -1, -2
    )
    ...
    if dift_type == "temporal":
        ...
        animations[0, 0] = plot_general_skeleton_correspondance(
            cond_ref["parents"], joint2color_ref, len(set(values)), positions_ref, "", "truebones", fps=20
        )
        animations[0, 1] = plot_general_skeleton_correspondance(
            cond_tgt["parents"], joint2color_tgt, len(set(values)), positions_tgt, "", "truebones", fps=20
        )
    else:
        ...
        animations = np.empty(shape=(1, 2), dtype=object)
        animations[0, 0] = plot_general_skeleton_correspondance(
            cond_ref["parents"], joint2color_ref, n_kinchains, positions_ref, "", "truebones", fps=20
        )
        animations[0, 1] = plot_general_skeleton_correspondance(
            cond_tgt["parents"], joint2color_tgt, n_kinchains, positions_tgt, "", "truebones", fps=20
        )
    if dift_type == "temporal":
...
def run_dift(args=None, cond_dict=None):
    ...
    batch, model_kwargs, motions, cond_dicts = create_batch_from_motion_paths(
        [args.sample_ref] + args.sample_tgt, cond_dict, args.temporal_window, t5_conditioner, max_joints=opt.max_joints
    )
