    ...
    for rep_i in range(args.num_repetitions):
        ...
        for i, motion in enumerate(sample):
            ...
            out_anim, _1, _2 = animation_from_positions(
                positions=global_positions, parents=parents, offsets=offsets, iterations=150
            )
            name_pref = "%s_rep_%d" % (object_type, rep_i)
            existing_npy_files = [
                filename for filename in os.listdir(out_path) if filename.startswith(name_pref) and filename.endswith(".npy")
            ]
            existing_mp4_files = [
                filename for filename in os.listdir(out_path) if filename.startswith(name_pref) and filename.endswith(".mp4")
            ]
