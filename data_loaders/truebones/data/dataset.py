...
from data_loaders.truebones.truebones_utils.motion_process import add_joint_augmentation, remove_joints_augmentation
    ...
    def __getitem__(self, item):
        ...
        if m_length < self.max_motion_length:
            motion = np.concatenate(
                [motion, np.zeros((self.max_motion_length - m_length, motion.shape[1], motion.shape[2]))], axis=0
            )
...
class Truebones(data.Dataset):
    def __init__(self, split="train", temporal_window=31, t5_name="t5-base", **kwargs):
        ...
        assert len(self.motion_dataset) > 1, (
