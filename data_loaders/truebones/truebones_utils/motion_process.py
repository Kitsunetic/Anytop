...
def get_foot_contact(positions, foot_joints_indices, vel_thresh):
    ...
    foot_contact_vel_map = np.where(
        np.logical_and(total_vel <= vel_thresh, np.abs(positions[1:, foot_joints_indices, 1]) <= FOOT_CONTACT_HEIGHT_THRESH),
        1,
        0,
    )
...
def get_common_features_from_T_pose(t_pose_bvh, object_type, face_joints=None):
    ...
    t_pose_anim, _1, _2 = animation_from_positions(
        positions=t_pose_positions, parents=t_pose_anim.parents, offsets=t_pose_anim.offsets, iterations=150
    )
    ground_height = None
    if object_type == "Dragon":
        ground_height = 0
    scaled, root_pose_init_xz, ground_height, scale_factor = process_anim(
        t_pose_anim, object_type, ground_height=ground_height, face_joints=face_joints
    )
    ...
    if object_type in ["Anaconda", "KingCobra"]:  # special handel for snakes
        ...
        suspected_foot_indices = [
            ...
            if "toe" in t_pos_names[i].lower()
            or "foot" in t_pos_names[i].lower()
            or "phalanx" in t_pos_names[i].lower()
            or "hoof" in t_pos_names[i].lower()
            or "ashi" in t_pos_names[i].lower()
        ]
        # edge cases
...
def get_hml_aligned_anim(
    ...
    if not isinstance(bvh_path, Animation):
        ...
        squared_positions_error[bvh_path] = (
            0  # np.sum((global_pos - new_global_pos) ** 2)/(anim.positions.shape[0]*anim.positions.shape[1])
        )
        ...
        processed_anim, _xz, _gh, _sf = process_anim(
            raw_anim, object_type, root_pose_init_xz, scale_factor, ground_height, face_joints=face_joints
        )
    else:
        names = list()
...
def get_motion(
    ...
    try:
        ...
        cont_6d_params, r_velocity, velocity, r_rot, global_positions = get_bvh_cont6d_params(
            new_anim, object_type, face_joints=face_joints
        )
...
def process_object(
    ...
    root_pose_init_xz, scale_factor, ground_height, offsets, foot_indices, tpos_rots, names, tpos_anim, face_joints = (
        get_common_features_from_T_pose(t_pos_path, object_type, face_joints=face_joints)
    )
    t_pos_motion, parents, max_joints, new_anim = get_motion(
        tpos_anim,
        FOOT_CONTACT_VEL_THRESH,
        object_type,
        max_joints,
    ...
    for f in bvh_files:
        ...
        while begin < anim_len:
            if anim_len - begin > 240:
                ...
                slice_ind = anim_len
            motion, parents, max_joints, new_anim = get_motion(
                f,
                FOOT_CONTACT_VEL_THRESH,
                object_type,
                max_joints,
            ...
            if motion is not None:
                ...
                plot_general_skeleton_3d_motion(
                    pjoin(save_dir, ANIMATIONS_DIR, name + "_from_ric.mp4"),
                    parents,
                    positions,
                    dataset="truebones",
                    title="",
                    fps=20,
                    face_joints=face_joints if face_joints is not None else FACE_JOINTS[object_type],
                    fc=fc,
                )
...
def add_joint_augmentation(data, mean, std):
    ...
    possible_joints_to_add = [
        j for j in range(1, n_joints) if np.count_nonzero(joints_relations[j] == 2) == 1 and joints_relations[j, 0] != 1
    ]
    ...
    # tpos_first_frame
    tpos_first_frame_augmented = np.vstack(
        [tpos_first_frame[:add_j], tpos_new_feats[None], tpos_j_feats[None], tpos_first_frame[add_j + 1 :]]
    ).copy()
    ...
    return (
        ...
        graph_dist,
        relations,
        tpos_first_frame_augmented,
        offsets,
        joints_names_embs_augmented,
        kinematic_chains,
        mean_augmented,
        std_augmented,
    )
...
def process_single_object_type(object_type, save_dir):
    ...
    cur_counter = files_counter
    files_counter, frames_counter, max_joints, object_cond = process_object(
        object_type, files_counter, frames_counter, max_joints, squared_positions_error, save_dir=save_dir
    )
    cond[object_type] = object_cond
    objects_counter[object_type] = files_counter - cur_counter
