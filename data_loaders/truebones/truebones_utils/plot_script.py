...
def plot_3d_motion(
    save_path, kinematic_tree, joints, title, dataset, figsize=(3, 3), fps=120, radius=3, vis_mode="default", gt_frames=[]
):
    matplotlib.use("Agg")
    ...
    def update(index):
        ...
        ax.dist = 7.5
        plot_xzPlane(
            MINS[0] - trajec[index, 0], MAXS[0] - trajec[index, 0], 0, MINS[2] - trajec[index, 1], MAXS[2] - trajec[index, 1]
        )
...
def plot_general_skeleton_correspondance(
    parents, joint2color, n_colors, joints, title, dataset, figsize=(7, 7), fps=120, radius=5
):
    matplotlib.use("Agg")
...
def plot_general_skeleton_kmeans(parents, centroid_indices, k, joints, title, dataset, figsize=(7, 7), fps=120, radius=5):
    ...
    def init():
        ...
        ax2.bar(
            range(n_frames),
            height=1,
            color=[colors[centroid_indices[i]] for i in range(len(joints))],
            width=1,
            align="edge",
            edgecolor="none",
        )
...
def plot_general_skeleton_3d_motion(
    save_path, parents, joints, title, dataset="truebones", figsize=(7, 7), fps=120, radius=5, face_joints=[], fc=None
):
