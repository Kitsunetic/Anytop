...
def n_joints_to_mask(n_joints, max_joints):
    mask = torch.arange(max_joints + 1, device=n_joints.device).expand(len(n_joints), max_joints + 1) < (
...
def truebones_collate(batch):
    ...
    lengthsmaskbatchTensor = (
        lengths_to_mask(lenbatchTensor, databatchTensor.shape[-1]).unsqueeze(1).unsqueeze(1)
    )  # unqueeze for broadcasting
    jointsnumbatchTensor = torch.as_tensor(jointsnumbatch)
    jointsmaskbatchTensor = (
        n_joints_to_mask(jointsnumbatchTensor, databatchTensor.shape[1]).unsqueeze(1).unsqueeze(1)
    )  # unqueeze for broadcasting

    collated_temporalmasksbatch = collate_tensors(temporalmasksbatch)
    maskbatchTensor = (
        length_to_temp_mask(collated_temporalmasksbatch, lenbatchTensor, collated_temporalmasksbatch[0].size(0) - 1)
        .unsqueeze(1)
        .unsqueeze(1)
    )  # unqueeze for broadcasting
    ...
    cond = {
        "y": {
            ...
            "lengths_mask": lengthsmaskbatchTensor,
            "tpos_first_frame": tposfirstframebatchTensor,
            "crop_start_ind": cropstartindTensor,
            "mean": meanbatchTensor,
            "std": stdbatchTensor,
        }
    }
