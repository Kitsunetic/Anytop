...
class AnyTop(nn.Module):
    def __init__(
        ...
        **kargs
        ...
        self.input_process = InputProcess(
            self.input_feats, self.root_input_feats, self.latent_dim, t5_out_dim, skip_t5=self.skip_t5
        )

        print("Graph transformer init")
        seqTransDecoderLayer = GraphMotionDecoderLayer(
        ...
        self.seqTransDecoder = GraphMotionDecoder(seqTransDecoderLayer, num_layers=self.num_layers, value_emb=self.value_emb)
    ...
    def forward(self, x, timesteps, get_layer_activation=-1, y=None):
        ...
        x = self.input_process(
            x, tpos_first_frame, y["joints_names_embs"], y["crop_start_ind"]
        )  # applies linear layer on each frame to convert it to latent dim
